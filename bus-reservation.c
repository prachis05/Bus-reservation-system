...
tkt *root = NULL;
int cost(tkt *r);              // calculates costs
...
tkt *reservationInfo(tkt *, int, int *); // Display Reservation Info
tkt *insert(tkt **r, int custID); // inserting a node
...
tkt *reservationInfo(tkt *r, int s, int *custIDmatched) // find function
{
  if (r == NULL)
    return NULL;
  tkt *presentnode = r;
...
tkt *insert(tkt **r, int custId)
{
  if (*r == NULL)
  {

    *r = (tkt *)malloc(sizeof(tkt));
    (*r)->PassnNo = custId;
    if (*r == NULL)
    {
      printf("No memory…");
